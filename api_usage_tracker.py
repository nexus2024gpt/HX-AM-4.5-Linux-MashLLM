# api_usage_tracker.py — HX-AM v4.5.3 (только облачные провайдеры)
...
# Живой список провайдеров живёт только в config/providers.json (gitignored).
# Раньше здесь был захардкожен DEFAULT_PROVIDERS с реальными ключами — убран,
# так как providers.json уже существует и является единственным источником
# правды при обычной работе; этот список использовался лишь при самом первом
# запуске, когда providers.json ещё не создан.
...
class APIUsageTracker:
    ...
    def _load_providers(self):
        ...
        try:
            ...
            logger.error(f"APITracker: ошибка загрузки конфига — {e}, используем defaults")
            self._providers = list(DEFAULT_PROVIDERS)
    ...
    def get_providers_for_role(self, role: str) -> List[ProviderConfig]:
        today = datetime.now(timezone.utc).date().isoformat()
        static_providers = [p for p in self._providers if p.enabled and role in p.roles]
        if not static_providers:
            return []
        ...
        return sorted(static_providers, key=sort_key)
    ...
    def get_stats(self) -> dict:
        ...
        for p in self._providers:
            ...
            providers_stats.append({
                ...
                "account": p.account, "model": p.model, "roles": p.roles,
        ...
        for s in providers_stats:
            ...
            if k not in acc_agg:
                acc_agg[k] = {
                    "provider": s["provider"], "account": s["account"],
                    "requests_today": 0, "tokens_in_today": 0,
                    "tokens_out_today": 0, "errors_today": 0,
                    "requests_total": 0, "key_count": 0,
                }
            a = acc_agg[k]
    ...
    def reset_today(self):
        ...
        for u in self._usage.values():
            u.requests_today = u.tokens_in_today = u.tokens_out_today = u.errors_today = 0
            u.last_reset_date = today
        self._save_usage()
